...
export default function AuthPage() {
    ...
    return (
        <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2">
            {/* Left — Branding */}
            <div className="hidden lg:flex flex-col justify-center p-16 bg-gradient-to-br from-black via-dark-700 to-dark-800 relative overflow-hidden">
                ...
                <div className="absolute w-[400px] h-[400px] bg-[radial-gradient(circle,rgba(191,0,255,0.06)_0%,transparent_70%)] -bottom-[80px] -left-[80px] pointer-events-none" />

                <div className="relative z-10 max-w-[480px]">
                    <Link to="/" className="inline-flex items-center gap-3 font-display text-3xl font-extrabold mb-12">
                        <span className="w-11 h-11 bg-gradient-to-br from-neon-blue via-neon-purple to-neon-pink rounded-xl flex items-center justify-center text-lg">🚀</span>
                    ...
                    <div className="flex flex-col gap-5">
                        {[
                            { icon: '📋', title: '7-Day Action Plans', desc: 'Personalized weekly roadmaps for your target roles', color: 'bg-neon-blue/10' },
                            { icon: '🎤', title: 'Mock Interviews', desc: 'Role-specific practice with real-time feedback', color: 'bg-neon-purple/10' },
                            { icon: '📊', title: 'Readiness Score', desc: 'Track progress with shareable cards & leaderboards', color: 'bg-neon-green/10' },
                        ].map((f, i) => (
                            <div key={i} className="flex items-center gap-4">
                                <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-xl shrink-0 ${f.color}`}>{f.icon}</div>
                                <div>
                    ...
                    <div className="flex bg-white/3 rounded-xl p-1 mb-8 border border-white/8">
                        {['login', 'register'].map((t) => (
                            <button key={t} onClick={() => setTab(t)}
                                className={`flex-1 py-3.5 rounded-lg text-[0.95rem] font-semibold transition-all capitalize ${tab === t ? 'bg-gradient-to-br from-neon-blue via-neon-purple to-neon-pink text-white shadow-md' : 'text-gray-500 hover:text-gray-300'}`}>
                                {t}
                            </button>
                        ))}
                    </div>
                        ...
                        {tab === 'login' ? (
                            <form onSubmit={handleLogin}>
                                ...
                                <div className="mb-5">
                                    ...
                                    <div className="relative">
                                        <input type={showPw.login ? 'text' : 'password'} className="form-input pr-12" placeholder="Enter your password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} required />
                                        <button type="button" className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-500 hover:text-white transition-colors text-lg" onClick={() => togglePw('login')}>
                                            {showPw.login ? '🙈' : '👁️'}
                                        </button>
                                    </div>
                                </div>
                                <button type="submit" disabled={loading} className="w-full py-4 bg-gradient-to-br from-neon-blue via-neon-purple to-neon-pink text-black font-bold text-base rounded-xl btn-glow transition-all active:scale-[0.97] disabled:opacity-60 flex items-center justify-center gap-2">
                            ...
                            <form onSubmit={handleRegister}>
                                ...
                                <div className="mb-5">
                                    <label className="block mb-2 text-sm font-medium text-gray-400 tracking-wide">Password</label>
                                    <div className="relative">
                                        <input type={showPw.reg ? 'text' : 'password'} className="form-input pr-12" placeholder="Min. 8 characters" value={regPassword} onChange={(e) => { setRegPassword(e.target.value); checkStrength(e.target.value) }} required minLength={8} />
                                        <button type="button" className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-500 hover:text-white transition-colors text-lg" onClick={() => togglePw('reg')}>
                                            {showPw.reg ? '🙈' : '👁️'}
                                        </button>
                                    </div>
                                    {regPassword && (
                                ...
                                <div className="mb-5">
                                    ...
                                    <div className="relative">
                                        <input type={showPw.regC ? 'text' : 'password'} className="form-input pr-12" placeholder="Re-enter your password" value={regConfirm} onChange={(e) => setRegConfirm(e.target.value)} required />
                                        <button type="button" className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-500 hover:text-white transition-colors text-lg" onClick={() => togglePw('regC')}>
                                            {showPw.regC ? '🙈' : '👁️'}
                                        </button>
                                    </div>
                                </div>
